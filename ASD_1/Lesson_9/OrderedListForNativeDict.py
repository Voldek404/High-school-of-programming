...
class OrderedList:
    ...
    def find(self, val):
        if self.__ascending:
            ...
            stop_condition = -1
        node = self.head
        while node is not None and self.compare(node.value[0], val[0]) != stop_condition:
            if self.compare(node.value[0], val[0]) == 0:
                return node
            node = node.next
        return None
